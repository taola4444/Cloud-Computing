...
function createUserPage(req, res) {
    ...
    } else if(exist(session.username)) {
            if(accsubmit == res)
...
function loginPage(req, res) {
    ...
    if (username && password  != null){
        if(libDB.check("shopbebe",query1) ==  req.query.username,req.query.username){
            res.render("pages/product", {title: "ATN-Shop LOGIN page", configHeader: configHeader , currpage: "Login"  });
        }else{
